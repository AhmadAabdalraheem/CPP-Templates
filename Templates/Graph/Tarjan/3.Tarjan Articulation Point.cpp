...
struct TarjanArticulationPoints {
    ...
    int get_components_after_removal(int u, int total_initial_components) {
    if (!is_cut[u]) {
        // If not a cut vertex, removing it only removes the node itself from its component
        return total_initial_components; 
    }
    
    // Formula: Initial_Components + (Number of DFS children 'v' with low[v] >= dfn[u]) - (1 if u is Root else 0)
    return total_initial_components + cut_children_count[u] - (is_root[u] ? 1 : 0);
  }
};

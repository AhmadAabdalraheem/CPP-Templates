...
{
    return x>=0 && x<n && y<m && y>=0 && v[x][y] != '#';
}
int ans = INT_MAX;

void init()
{
    unsafe.assign(n, vector<bool>(m, false));
...
void dfs(int r, int c, int steps) {
    ...
    visited[r][c] = true;
    if (v[r][c] == '.' || v[r][c] == 'P' || v[r][c] == '#') 
    {
        for (int k = 0; k < 4; ++k) {
            int nr = r + dx[k];
            int nc = c + dy[k];
            if (isvalid(nr,nc) && !unsafe[nr][nc] && !visited[nr][nc] )
            {
                dfs(nr, nc, steps + 1);
            }
        }
    } 
    else if (v[r][c] >='1' && v[r][c] <='9')
     {
        int jump = v[r][c] - '0';
        for (int k = 0; k < 4; k++) {
            int nr = r + dx[k] * jump;
